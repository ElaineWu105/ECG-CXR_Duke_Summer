...
def make_learnable_weight_model(base_class, initial_lambda):
    class Model(base_class):
        def __init__(self, *args, **kwargs):
            super().__init__(*args, **kwargs)
            self.log_var_instance = nn.Parameter(torch.tensor(0.0))
            self.log_var_label = nn.Parameter(
                torch.tensor(-float(np.log(initial_lambda))))
    return Model
...
def make_run_epoch(lambda_label: float, label_top_k: int, learnable_weights=False):
    def run_epoch(model, loader, optimizer, device, w_cross, w_temporal, max_grad_norm,
        ...
        for iter_in_epoch, batch in enumerate(loader, start=1):
            ...
            if learnable_weights:
                instance_weight = torch.exp(-model.log_var_instance)
                label_weight = torch.exp(-model.log_var_label)
                effective_lambda = label_weight / instance_weight
                loss = (instance_weight * base + model.log_var_instance
                        + label_weight * label_loss + model.log_var_label)
            else:
                instance_weight = base.new_tensor(1.0)
                label_weight = base.new_tensor(float(lambda_label))
                effective_lambda = label_weight
                loss = base + label_weight * label_loss
            ...
            if iteration_records is not None and global_step % max(1, dynamics_log_every) == 0:
                iteration_records.append({
                    ...
                    "instance_weight": float(instance_weight.detach()),
                    "label_weight": float(label_weight.detach()),
                    "effective_lambda": float(effective_lambda.detach()),
        ...
        if learnable_weights:
            instance_weight = float(torch.exp(-model.log_var_instance.detach()))
            label_weight = float(torch.exp(-model.log_var_label.detach()))
        else:
            instance_weight, label_weight = 1.0, float(lambda_label)
        effective_lambda = label_weight / instance_weight
        result = {
        ...
        if learnable_weights:
            print(f"      learned weights: instance={instance_weight:.4f} "
                  f"label={label_weight:.4f} "
                  f"effective_lambda={effective_lambda:.4f}", flush=True)
        return result
    return run_epoch
...
def main():
    ...
    extra.add_argument("--learnable_loss_weights", action="store_true")
    ...
    if custom.learnable_loss_weights:
        engine.StagedModel = make_learnable_weight_model(
            engine.StagedModel, custom.lambda_label)
    engine._run_epoch = make_run_epoch(
        custom.lambda_label, custom.label_top_k, custom.learnable_loss_weights)
    runner.main()
