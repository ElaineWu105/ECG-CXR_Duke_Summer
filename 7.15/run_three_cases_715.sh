#!/bin/bash
#SBATCH -J cross-715-antioverfit
#SBATCH -t 72:00:00
...
#SBATCH --mem=128G
#SBATCH -G 1
#SBATCH -o ./7.15/logs/%x-%j.out
#SBATCH -e ./7.15/logs/%x-%j.err

# 7.15 anti-overfitting rerun of the 7.13 three-case grid.
# Uses 7.15/staged_model.py via 7.15/run_case_experiment.py.
set -euo pipefail
...
EMB="${EXP}/cache"
OUT="${ROOT}/7.15/cross_patient_huge_batch_antioverfit_seqtokdrop"
WRAPPER="${ROOT}/7.15/run_case_experiment.py"

source "${EXP}/setup_env.sh"
mkdir -p "${OUT}" "${ROOT}/7.15/logs"
...
run_one() {
  ...
  echo "===== 7.15 Case ${case_no}, n=${n}, output=${root} ====="
...
echo "Running 7.15 anti-overfitting experiments."
echo "Output directory: ${OUT}"
echo "Model override: ${ROOT}/7.15/staged_model.py"
echo "Regularization: sequence_token_drop=0.20, seq_pool_dropout=0.30, emb_drop=0.50"
...
for n in "${N_VALUES[@]}"; do
  ...
  run_one 2 "${n}" exp3a_seq_ecg_meanpool \
    ...
    "All ECGs in the shifted 12-hour window form one sequence-CXR pair; cross-patient only; 7.15 sequence regularization enabled."
...
echo "7.15 anti-overfitting run complete: ${OUT}"
