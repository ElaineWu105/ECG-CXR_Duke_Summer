...
EMB="${EXP}/cache"
OUT="${ROOT}/7.18/window_change_cls_n024"
[[ -f "$EXP/setup_env.sh" ]] && source "$EXP/setup_env.sh"
mkdir -p "${OUT}" "${ROOT}/7.18/logs"
for N_VALUE in 0 2 4; do
  export N_VALUE
  NAME="case2_no_time_window_change_cls_n${N_VALUE}"
  [[ "${SKIP_EXISTING:-1}" == 1 && -f "${OUT}/${NAME}/results.json" ]] && continue
  echo "===== no-time global-window change CLS n=${N_VALUE} ====="
  python3 -u "${ROOT}/7.18/train_window_change_cls.py" \
